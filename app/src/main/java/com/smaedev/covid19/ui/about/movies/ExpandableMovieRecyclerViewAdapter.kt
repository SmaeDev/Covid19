 ...
 * @author Mobin Munir
 */
abstract class ExpandableMovieRecyclerViewAdapter<ExpandedType : Any, ExpandableType : ExpandableMovieRecyclerViewAdapter.ExpandableGroup<ExpandedType>, PVH : ExpandableMovieRecyclerViewAdapter.ExpandableViewHolder, CVH : ExpandableMovieRecyclerViewAdapter.ExpandedViewHolder>
 ...
 */
    ...
    private fun initializeChildVideoView(childVideoView: VideoView?) {

        if (childVideoView != null) {
            ...
            childVideoView.setVideoURI(Uri.parse(vidAddress))
            /*childVideoView.requestFocus()

            childVideoView.setOnCompletionListener(OnCompletionListener { })

            childVideoView.setOnPreparedListener(OnPreparedListener { mediaPlayer ->
                childVideoView.start()
                mediaPlayer.setOnVideoSizeChangedListener { mp, width, height ->
                    val mediaController = MediaController(MainActivity.mainContext())
                    childVideoView.setMediaController(mediaController)
                    mediaController.setAnchorView(childVideoView)
                }
            })

            childVideoView.setOnErrorListener(OnErrorListener { mediaPlayer, i, i1 -> false })*/

            childVideoView.start()
            /*val vidControl = MediaController(MainActivity.mainContext())
            vidControl.setAnchorView(childVideoView);
            childVideoView.setMediaController(vidControl);*/
    ...
    private fun onCreateParentView(parent: ViewGroup, viewType: Int): PVH {
        ...
        initializeChildVideoView(pvh.containerView.getVideoView())
    ...
    private fun clickEvent(expandableGroup: ExpandableType, containerView: View) {
        val childVideoView = containerView.getVideoView()

        childVideoView?.visibility = if (expandableGroup.isExpanded)
            View.VISIBLE
        else View.GONE
    ...
    private fun View.getVideoView(): VideoView? {
        if (this is ViewGroup && childCount > 0) {
            forEach {
                if (it is VideoView) {
                    return it
    ...
    abstract class ExpandedViewHolder(override val containerView: View) : VideoViewBindingAdapter(), LayoutContainer
    ...
    abstract class ExpandableGroup<out E> {
         ...
         */
        abstract fun getExpandingItems(): VideoView
